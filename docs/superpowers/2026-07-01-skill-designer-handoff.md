...
## ADDENDUM 2 (2026-07-02): portable dev env — git deps + nix flake (DONE + PUSHED)

The `../` sibling path-dep layout is no longer required to BUILD. See `specs/2026-07-02-portable-dev-env-design.md` + `plans/2026-07-02-portable-dev-env-plan.md`, and obelisk-arena's `DEVELOPING.md` (the canonical setup doc). Highlights: all cross-repo deps are git deps (obelisk is PRIVATE → `ssh://` URLs, SSH key needed to build); `LukeThayer/obelisk-arena` + `LukeThayer/bevy_modal_editor` (fork of Nub's, `upstream` kept) now exist; bevy_egui pinned to rev `81904dac…` in manifests; `tools/dev-siblings.sh` writes git-ignored config-level `[patch]` files for the co-dev loop (never commit patched locks); `flake.nix` devShell (linux+darwin) verified on this mac — the "nix unusable" gotcha above is OBSOLETE (it was bevy_modal_editor's graphviz devShell). Never-push convention retired for these 4 repos. Proof gate: bare temp-dir clone built both workspaces with no siblings.
