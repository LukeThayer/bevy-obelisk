...
fn server_drains_serializable_netevents() {
    ...
    let count_cast_began = collected.iter().filter(|e| matches!(e, NetEvent::CastBegan { .. })).count();
    let count_damage = collected.iter().filter(|e| matches!(e, NetEvent::DamageResolved { .. })).count();
    let count_effect_applied = collected.iter().filter(|e| matches!(e, NetEvent::EffectApplied { .. })).count();
    let count_effect_expired = collected.iter().filter(|e| matches!(e, NetEvent::EffectExpired { .. })).count();
    let count_dot = collected.iter().filter(|e| matches!(e, NetEvent::DotTicked { .. })).count();
    let count_died = collected.iter().filter(|e| matches!(e, NetEvent::EntityDied { .. })).count();
    println!(
