...
#[test]
fn cue_event_carries_originating_skill_id() {
    let mut t = ObeliskTestApp::new(43);
    let handle: Handle<CastTimeline> = t
        .app
        .world()
        .resource::<AssetServer>()
    ...
    let fired_skill_ids: Arc<Mutex<Vec<String>>> = Arc::new(Mutex::new(Vec::new()));
    let f1 = fired_skill_ids.clone();
    t.app.observe_cue("firebolt_cast", move |cue, _cmds| {
        f1.lock().unwrap().push(cue.skill_id.clone())
    });
    ...
    let fired_skill_ids = fired_skill_ids.lock().unwrap();
    assert!(
        fired_skill_ids.contains(&"firebolt".to_string()),
        "on_cast CueEvent should carry skill_id == \"firebolt\", got {:?}",
        *fired_skill_ids
    );
    let _ = dummy;
}
